...
public class Config extends AbstractConfig<NTheEndAgain> {
    ...
    // Data
    // Used for Regen task timer persistence, based on System.currentTimeMillis()
    private static final long DEFAULT_nextRegenTaskTime = 0;
    private long nextRegenTaskTime;

    // Used for Respawn task timer persistence, based on System.currentTimeMillis()
    ...
    private long lastOuterEndRegenTime;

    // Keep track of regen counts
    private int centralEndRegenCount;
    private int outerEndRegenCount;
    ...
    public Config(final NTheEndAgain instance, final String world) {
        ...
        // Data
        this.setNextRegenTaskTime(DEFAULT_nextRegenTaskTime, "Config constructor");
        this.setNextRespawnTaskTime(DEFAULT_nextRespawnTaskTime, "Config constructor");
        this.setLastOuterEndRegenTime(System.currentTimeMillis(), "Config constructor");

        this.setCentralEndRegenCount(0);
        this.setOuterEndRegenCount(0);
    }
    ...
    protected void handleValues(final YamlConfiguration config) {
        ...
        // When taskTimeVersion is 0 or 1, we assume that nextRegenTaskTime and nextRespawnTaskTime are based on System.nanoTime
        // When that is true, the values are divided by 1 million so that we can use System.currentTimeMillis()

        int taskTimeVersion = config.getInt("taskTimeVersion", 1);
        long currentTime = System.currentTimeMillis();

        long nextRegenTaskTime = config.getLong("nextRegenTaskTime", DEFAULT_nextRegenTaskTime);
        if (taskTimeVersion <= 1) {
            // Convert from nanoTime to System.currentTimeMillis()
            nextRegenTaskTime = NanoTimeToMilliTime(currentTime, nextRegenTaskTime);
        }

        this.setNextRegenTaskTime(nextRegenTaskTime, "Config.handleValues");
        if (!this.match(this.nextRegenTaskTime, 0, Long.MAX_VALUE)) {
            ...
            this.setNextRegenTaskTime(DEFAULT_nextRegenTaskTime, "Config.handleValues");
        }

        long nextRespawnTaskTime = config.getLong("nextRespawnTaskTime", DEFAULT_nextRespawnTaskTime);
        if (taskTimeVersion <= 1) {
            // Convert from nanoTime to System.currentTimeMillis()
            nextRespawnTaskTime = NanoTimeToMilliTime(currentTime, nextRespawnTaskTime);
        }

        this.setNextRespawnTaskTime(nextRespawnTaskTime, "Config.handleValues");
        if (!this.match(this.nextRespawnTaskTime, 0, Long.MAX_VALUE)) {
            ...
            this.setNextRespawnTaskTime(DEFAULT_nextRespawnTaskTime, "Config.handleValues");
        }

        // If the last outer end regen time was missing or is 0, update it to currentTime
        long lastOuterEndRegenTime = config.getLong("lastOuterEndRegenTime", 0);
        if (lastOuterEndRegenTime < 1)
            lastOuterEndRegenTime = currentTime;

        this.setLastOuterEndRegenTime(lastOuterEndRegenTime, "Config.handleValues");
        if (!this.match(this.lastOuterEndRegenTime, 0, Long.MAX_VALUE)) {
            this.wrongValue(fileName, "lastOuterEndRegenTime", this.lastOuterEndRegenTime, currentTime);
            this.setLastOuterEndRegenTime(currentTime, "Config.handleValues");
        }

        this.setCentralEndRegenCount(config.getInt("centralEndRegenCount", 0));

        this.setOuterEndRegenCount(config.getInt("outerEndRegenCount", 0));
    }

    private long NanoTimeToMilliTime(long currentTime, long nanoBasedTime) {
        long milliBasedTime = currentTime + (nanoBasedTime - System.nanoTime()) / 1000000L;
        if (milliBasedTime < currentTime)
            milliBasedTime = currentTime;

        return milliBasedTime;
    }
    ...
    protected String getConfigString() {
        ...
        content.append("lastOuterEndRegenTime: ").append(this.lastOuterEndRegenTime).append("\n\n");

        // Regen Counts
        content.append("# Regeneration stats\n");
        content.append("centralEndRegenCount: ").append(this.centralEndRegenCount).append("\n");
        content.append("outerEndRegenCount: ").append(this.outerEndRegenCount).append("\n\n");
    ...
    public void setNextRegenTaskTime(final long newTaskTime, String callingFunction) {
        if (newTaskTime != 0 || newTaskTime == 0 && this.nextRegenTaskTime != 0)
            this.plugin.debug("nextRegenTaskTime     -> " + newTaskTime + " via " + callingFunction);
        this.nextRegenTaskTime = newTaskTime;
    }
    ...
    public void setNextRespawnTaskTime(final long newTaskTime, String callingFunction) {
        if (newTaskTime != 0 || newTaskTime == 0 && this.nextRespawnTaskTime != 0)
            this.plugin.debug("nextRespawnTaskTime   -> " + newTaskTime + " via " + callingFunction);
        this.nextRespawnTaskTime = newTaskTime;
    }

    public long getLastOuterEndRegenTime() {
        return this.lastOuterEndRegenTime;
    }

    public void setLastOuterEndRegenTime(final long newRegenTime, String callingFunction) {
        if (newRegenTime != 0 )
            this.plugin.debug("lastOuterEndRegenTime -> " + newRegenTime + " via " + callingFunction);
        this.lastOuterEndRegenTime = newRegenTime;
    }
    ...
    public long getSecondsUntilNextExpectedRespawn() {
        ...
        long timeRemainingSec = (this.nextExpectedRespawnTime - System.currentTimeMillis()) / 1000;
    ...
    public void updateNextExpectedRespawnTime(long respawnDelaySeconds) {
        this.nextExpectedRespawnTime = System.currentTimeMillis() + respawnDelaySeconds * 1000;
    }
}
